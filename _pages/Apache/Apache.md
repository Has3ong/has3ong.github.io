---
layout : single
permalink: /ap/
title: "Apache"
sidebar:
  # nav: sidebar-ML
header:
  # overlay_image: /assets/images/category/ml.jpg
---

## Zookeeper

* [Zookeeper 소개](/zookeeper-introduction)

## Kafka

* [Kafka 소개](/kafka-introduction)
* [Meet Kafka](/kafka-intro1)
* [Installing and Configuring Kafka -1- Zookeeper / Kafka Install](/kafka-install1)
* [Installing and Configuring Kafka -2- Kafka Server Properties](/kafka-install2)
* [Kafka Producers -1- Producer](/kafka-producer1)
* [Kafka Producers -2- Producer Properties / acks](/kafka-producer2)
* [Kafka Producers -3- Serializer](/kafka-producer3)
* [Kafka Producers -4- Avro / Partition](/kafka-producer4)
* [Kafka Consumers -1- Consumer / Consumer Group](/kafka-consumer1)
* [Kafka Consumers -2- Poll Loop / Consumer Properties](/kafka-consumer2)
* [Kafka Consumers -3- Commit Offsets](/kafka-consumer3)
* [Kafka Consumers -4- Rebalance Listeners](/kafka-consumer4)
* [Kafka Consumers -5- Deserializers](/kafka-consumer5)
* [Kafka Internals -1- Cluster Membershio / Controller](/kafka-internals1)
* [Kafka Internals -2- Replication / ISR](/kafka-internals2)
* [Kafka Internals -3- Request Processing](/kafka-internals3)
* [Kafka Internals -4- Storage / File Management](/kafka-internals4)
* [Kafka Internals -5- Compaction](/kafka-internals5)

## Hadoop

* [Installing Hadoop on macOS](/hadoop-install)
* [The Hadoop Distributed Filesystem](/hadoop-introduction1)

## Spark

* [Spark 소개](/spark-introduction)
* [pyspark 설치하기](/pyspark-install)

## Reference

* [zookeeper.apache.org](https://zookeeper.apache.org/)
* [kafka.apache.org](https://kafka.apache.org/)
* [spark.apache.org](https://spark.apache.org/)
* [Kafka 한국 사용자 모임에서 운영하는 meetup repository](https://github.com/kafkakru/meetup)
